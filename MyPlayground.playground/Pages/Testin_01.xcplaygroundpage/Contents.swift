...
class MovementForward_01: Movements {
    
    override func performAction(movementDirection: MovementDirections, position: inout Position)
    {
...
class TurnClockwise_01: Movements {
...
class TurnCounterClockWise_01: Movements {
...
//        var mover = Movements()
        let moveForward = MovementForward_01()
        let moveleft = TurnClockwise_01()
        let moveright = TurnCounterClockWise_01()
        
        for operation in operations {
            switch Operation(rawValue: operation) {
                case .P:
                
                moveForward.performAction(movementDirection: position.movementDirection, position: &position)
                    break
                case .L:
                
                moveleft.performAction(movementDirection: position.movementDirection, position: &position)
                    break
                case .D:
                
                moveright.performAction(movementDirection: position.movementDirection, position: &position)
                    break
                default:
                    break
            }
...
final class Robot {
    ...
    // Create array which holds iterations of permutated positions of the operation strings
        ...
        heapPermutation(data: &movementsPositions) { result in
            ...
            position = moveRobot(move: movements[index], previousPosition: position, previousDirection: position.movementDirection)
            }
